...
3. Add regression tests to `installer/design_test.go` mirroring `TestParseDesignRejectsProseAndUnsafeIncludes`'s style:
   ...
   - Accepts a true black-box form (one actor, one system participant identified as `System`) — confirm the first pilot run's `Customer`/`System` diagram (aliased `as System`) still passes.
