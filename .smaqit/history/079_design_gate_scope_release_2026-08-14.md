...
- Scrubbed the downstream project name out of task 109 and task 110's files per `CONTRIBUTING.md`'s naming rule — genericized task 109's mentions, and replaced task 110's (where the real name was load-bearing evidence for a slug-truncation bug) with an illustrative fictional project name that preserves the same bug mechanics.
