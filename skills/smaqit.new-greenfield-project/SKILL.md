...
3. Invoke `/smaqit.deployment` agent with context: generate all IaC artifacts — Terraform files in `deployment/terraform/` and GitHub Actions workflow files in `.github/workflows/` using `smaqit.infrastructure-cicd-generate` patterns as reference; Terraform state key `dev/terraform.tfstate`; do not trigger deployment execution.
   ...
   → **`existing-k3s`:** invoke `smaqit.infrastructure-cicd-generate` in `k3s` mode. No Terraform files are generated; `provision.yml` is not generated; `deploy.yml` has a single kubectl-driven `deploy` job (guard → lint → apply → rollout → verify) reading `KUBECONFIG` from a GitHub Environment secret. No SSH, no nginx, no `VM_HOST` anywhere in the generated output.
...
6. Resolve and invoke the deploy skill for the project's stack. The `provisioning_mode` selects
   ...
   → **`existing-k3s`:** before invoking the deploy skill, invoke `smaqit.infrastructure-request-k3s-onboarding` for the test environment's registered machine-slug and gate on its PR merging — the deploy skill's own Pre-conditions require the Namespace to already be onboarded, and this is the step that requests it. If the gate reports "pending," stop this task here (re-invoke later — it re-checks the same PR rather than re-creating it) rather than proceeding to the deploy skill against a Namespace that doesn't exist yet. Once merged, invoke the matched or synthesized `smaqit.infrastructure-deploy-k3s-app*` skill against the `test` Namespace/kubeconfig. There is no nginx vhost step at all — routing is the shared Ingress template (rendered with `__APP_SLUG__`, the declared ingress class, and `ClusterIssuer`), not a VM vhost. The skill's own `namespace-guard.sh` and `manifest-lint.sh` run before any `kubectl apply`, and the skill invokes `smaqit.infrastructure-deploy-verify` itself as its final step — Step 7 below still runs, but only re-confirms what the deploy skill already checked.
...
3. Invoke `smaqit.infrastructure-repo-config` to sync all production secrets from Vault to GitHub Secrets (cloud credentials, Terraform backend, SSH key).
   ...
   → **`existing-k3s`:** `repo-config` writes the `prod` kubeconfig as a `KUBECONFIG` secret on the `prod` GitHub Environment (not a plain repository secret — see `smaqit.infrastructure-repo-config`). No `VM_HOST` is ever set, and no `tfstate`/`cyso`/`ssh`-derived secret is synced.
4. Push to main: `git push origin main`. The `deploy.yml` workflow triggers automatically (provision job → deploy job).
   ...
   → **`existing-k3s`:** `deploy.yml` has a single kubectl-driven `deploy` job (guard → lint → apply → rollout → `Certificate` → verify) — generated that way by `smaqit.infrastructure-cicd-generate`'s `k3s` mode in Phase 4. No `provision` job, no SSH, no Terraform reference anywhere in the generated workflow.
