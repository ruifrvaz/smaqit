...
- **Post-completion correction (2026-09-12):** a concurrent session corrected task 116's own scope mid-PR-review — its original detailed mechanism (registry file + converge workflow, specific RBAC/PSA/NetworkPolicy mechanics) was found to be wrongly hardcoded as a universal smaqit contract, and the compiled `smaqit.infrastructure-onboard-k3s-app` was rewritten as a thin dispatcher deferring entirely to whatever onboarding mechanism the target infra repo actually uses. This task's own design (PR-to-a-registry-file, gate-on-merge) was unaffected as a mechanism, but its docs incorrectly cited task 116 as documenting that mechanism. Corrected in a follow-up commit on this branch: reframed the PR-request model as smaqit's own declared, opinionated app-side convention (legitimate specifically because this skill lives in the smaqit-managed app repo, not the infra repo whose internals vary per-target), with an explicit new precondition that the target infra repo's onboarding process must accept requests via a PR-mergeable registry file for this skill to apply. User confirmed this reasoning directly and it required no other change to task 117 or 118's actual mechanism.
