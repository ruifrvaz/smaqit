...
### Fixed
- **Missing existing-k3s Infrastructure-spec fields** (pending v3.5.2 · PR #90) — surface the
  required `Platform Repo`, `Registry File Path`, `Machine Slug`, `Ingress Class`, `ClusterIssuer`,
  and `Namespace Quota` fields for an `existing-k3s` deployment target and fail loudly instead of
  silently when they're missing, instead of assuming they exist.
...
### Added
- **k3s app-deployment target** — new `smaqit.infrastructure-deploy-k3s-app` skill deploys an
...
- **k3s app-onboarding request skill** — new `smaqit.infrastructure-request-k3s-onboarding` lets
...
### Changed
- **Vault namespace convention extended for kubeconfig credentials** —
...
### Fixed
- **Vault loader project-slug derivation** — project slug now derives from `git remote get-url origin` (basename, `.git` stripped) or the current directory name, via one shared function used by both `load-credentials.sh` and `rotate-credential.sh`, instead of parsing `AGENTS.md`'s human-readable "Project Name" field, which inconsistently truncated multi-word heading-format titles.
- **Vault loader non-interactive secret-write safety** — all 7 confirmed ad hoc, non-`/dev/tty` secret reads across `load-credentials.sh` and `rotate-credential.sh` now use the existing `read_secret` helper, so a non-interactive invocation fails loudly instead of silently writing an empty or placeholder secret to Vault. An empty-value guard is also checked immediately before every affected `vault kv put`.

### Chore
- **Removed 18 leftover ADK-generic duplicate template/rules files** under `templates/` and `.smaqit/templates/` that escaped an earlier cleanup pass and were never read by any generator.
...
### Added
- Design Sequence Diagrams — a new post-implementation artifact category generated by `smaqit.development` after code and tests pass, one per implemented Functional spec, pairing 1:1 with the spec's `system-sequence` SSD. Stored in `docs/designs/design-sequence/` with full CLI parity (render/attest/validate). Two deterministic checks wired into `smaqit design attest`: **grounding** (every message must cite a real `file:line`) and **completeness** (every operation the paired SSD promises must appear in the diagram).

### Changed
- `framework/ARTIFACTS.md` — new Design Sequence Diagrams section with ownership (Development), storage, content boundary, and a lifecycle without the reset-to-draft clause
- `framework/PHASES.md` — Phase 1 table, completion criteria, and Develop→Deploy prerequisites now include design-sequence diagrams
- `agents/development.md` — new MUST directive for generating and validating design-sequence diagrams; design-authoring restriction scoped explicitly to Design Artifacts only
...
### Chore
-  ReBumped release v2.2.0 for rerun.
