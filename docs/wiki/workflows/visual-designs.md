...
Initialization and project update automatically run `smaqit mcp verify` to prove VS Code, Claude Code, and Codex project registration plus local stdio transport. Re-run it manually to diagnose that deterministic local layer. It does not prove an interactive client has exposed the tools: open the project in VS Code and use **MCP: List Servers** to trust/start `smaqit-plantuml`; in Claude Code, restart and trust the project `.mcp.json`; in Codex, restart after trusting the project so it can load `.codex/config.toml`. Known Codex host defects can still ignore a valid trusted project registration. Start a fresh specification-agent session and confirm it receives its two declared tools. If they are absent, stop authoring with `DESIGN-TOOLCHAIN-UNAVAILABLE`; do not substitute direct CLI calls.
...
The released Go binary embeds exact npm-lock-resolved dependencies for `@plantuml/mcp-js`, `@resvg/resvg-wasm`, and Noto Sans. Initialization first verifies the archive and Node.js 22+, then materializes the versioned runtime under `.smaqit/tools/plantuml/`. It adds the narrow `.smaqit/tools/` rule to the project root `.gitignore`, preserving user rules so the generated runtime is not committed; canonical `docs/designs/` Markdown and PNG artifacts remain versioned. It configures the owned `smaqit-plantuml` server in `.vscode/mcp.json` for VS Code, root `.mcp.json` for Claude Code, and trusted `.codex/config.toml` for Codex. Agent declarations constrain an agent's allowed tools; they do not register a host MCP server.
