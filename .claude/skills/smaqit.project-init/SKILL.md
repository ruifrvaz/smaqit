...
Bootstrap a new smaqit project by generating `CLAUDE.md` from the smaqit template.
...
1. **Check for existing file**
   ...
   - If it **does exist**, stop immediately and inform the user:
     > `CLAUDE.md` already exists. Aborting to avoid overwriting your project instructions. Delete or rename the existing file and run this skill again if you want to reinitialise.
...
5. **Write the output file**
   - Write the populated content to `CLAUDE.md`.
   - Confirm success to the user and list which fields were filled in and which were left as placeholders:
     > ✓ `CLAUDE.md` created successfully. Review and commit the file to include it in your project.
...
- **Never overwrite** an existing `CLAUDE.md` (Step 1 is a hard guard).
...
- Do **not** create any additional files beyond `CLAUDE.md`.
