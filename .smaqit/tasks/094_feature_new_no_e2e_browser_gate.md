...
- [ ] The detection mechanism is based on the real Development diff, not solely the task file's own predicted file list, to avoid the same near-miss that occurred on `iodis-crm-poc`
...
- [ ] A new Gotcha or note references `iodis-crm-poc` task 016 as the concrete case that surfaced this gap, following the same documentation precedent as Task 092
