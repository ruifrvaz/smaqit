...
- [ ] `deploy.yml` written (provision + deploy jobs, sequential, `plan-guard.sh` gates the apply, no GITHUB_TOKEN collision)
- [ ] `provision.yml` written (PR-only, plan only via `plan-guard.sh`, path filter, `TF_VAR_github_token` present)
...
| Generated `deploy.yml` has no plan-guard gate in front of `apply` | Add the `plan-guard.sh` invocation step before `terraform apply`; this is not optional |
