...
2. **Assess each required section** — Determine if content is specific enough to produce testable behavior descriptions
3. **Elicit gaps** — Ask the targeted question for each insufficient section; collect one section at a time
4. **Confirm readiness** — Once all required sections are satisfied, proceed directly to Functional spec generation
