---
name: smaqit.infrastructure-deploy-rsync-python-tornado
description: Use when deploying a Python/Tornado application (single-process monolith, no Docker, no build step, systemd-managed, SQLite persistence) to a remote VM via rsync. For stacks that match neither `smaqit.infrastructure-deploy-rsync` (Node.js + Vite/React, Docker Compose) nor `smaqit.infrastructure-deploy-rsync-python-nextjs` (FastAPI + Next.js, Docker Compose) — this variant has no container runtime, no frontend build, and no `backend/`/`frontend/` split. Used in the Phase 4 dev environment sweep of `smaqit.new-greenfield-project`. Also usable as a manual fallback for direct VM deployment outside the CI/CD pipeline.
metadata:
  version: "1.0.0"
  validated: "2026-07-21"
  validated-stack: "Python 3.12, Tornado 6.4, SQLite (stdlib), nginx 1.24, systemd 255, no Docker, no build step"
---

# Deploy Python/Tornado Application via rsync

Validated path for deploying a single-process Python/Tornado monolith (no Docker, no frontend
build, SQLite persistence) to a remote VM via rsync.

## Pre-conditions

- VM bootstrapped (data volume mounted, app directory owned by `ubuntu`, nginx installed and
  enabled, `python3-tornado` installed via apt — see `smaqit.infrastructure-vm-bootstrap`; that
  skill's packaged steps target a Docker+Node stack and must be manually adapted for this profile)
...
5. **Write deploy stamp files** (enables SHA verification in the health endpoint):
   ...
   never `echo` (which appends a newline the health endpoint would then include).
...
**Input:** Python/Tornado + SQLite + nginx + systemd stack; Phase 4 dev sweep of
`smaqit.new-greenfield-project` (or a manual operator invocation) determines this profile matches
the declared stack and invokes this skill directly — no synthesis required.

**Output:** `src/` rsynced to `<host>:__APP_DIR__/app/` (excluding `*.db`), systemd unit installed
and the application service restarted, nginx vhost written via `write-vhost.sh`, deploy stamps
written, health endpoint returns correct SHA.
...
- **No Docker anywhere** — resist the temptation to add a Docker step by analogy with sibling
  deploy skills; this stack deliberately has none.
