...
| Functional | `system-sequence` | `domain-model`, `context-map`, `state` when domain boundaries, entities, or lifecycle behavior exist. `system-sequence` itself stays a strict black box: exactly one explicit actor, exactly `participant "System" as System` (case-insensitive), `hide footbox`, and no undeclared message endpoints (enforced at validation). Author one design per actor or per user-story/flow whenever a spec involves more than one. |
