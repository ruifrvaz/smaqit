# Reconcile a Downstream Python/FastAPI + Next.js Deploy Skill Into Canonical smaqit
...
against a real downstream project (a FastAPI 0.115 + Next.js 15 + pnpm +
...
`smaqit.infrastructure-*` skill between this repo and the downstream project's install also surfaced a
...
the downstream project is ahead by one small convenience in `vault-loader`'s credential script). This
task reconciles the Python/Next.js deploy skill and the smaller upstream-bound fix into this repo.
It does not attempt to push this repo's own fixes back down into the downstream project's installed
copy — see Design Decisions.

## Design Decisions

- **Direction of reconciliation: downstream project → smaqit canonical, one way, this task.**
  Pushing this repo's own fixes (the `vm-bootstrap` / `provider-cyso` refinements
  the downstream project is missing) back down into that project's installed copy is explicitly out of
  scope here — that happens naturally the next time that project runs its own
...
- **Merge Phase 4 Step 6 branching without regressing Task 084's `provisioning_mode` branching.**
  the downstream project's copy of `new-greenfield-project` is a pre-Task-084 snapshot (v1.0.0) and
...
- **Fix the stray third-party project reference while importing.** `run-migrations.sh`'s own usage
  example references a specific, unrelated project's deploy path — neither the downstream project nor
...
1. **Import the skill**: copy
   `<downstream-project>/.github/skills/smaqit.infrastructure-deploy-rsync-python-nextjs/` (`SKILL.md`
...
5. **Update the "Source path contract" Gotcha** in `smaqit.new-greenfield-project` to mention
   stack-based deploy skill selection (matching the downstream project's own wording), reconciled with
...
- [x] This task does NOT modify anything under the downstream project's directory — the
      reconciliation is one-directional, into this repo only (verified via `git -C
      .../<downstream-project> status --short`: only that project's own pre-existing, unrelated local
...
- Verified the one-directional constraint explicitly at the end via `git -C
  .../<downstream-project> status --short` in a separate invocation (an earlier combined-invocation
...
- Two follow-ups already flagged in this task's own Notes (not part of this task): a Python/Next.js
  ...
  `provider-cyso` refinements down into the downstream project's installed copy.
...
- Originates from a live diff against a downstream project's `.github/skills/`,
  triggered by questioning whether Task 085's Node.js-only templates were too rigid. Confirmed via
  the project's `backend/pyproject.toml` and `frontend/package.json` that its real stack is
...
- **Follow-up identified, not part of this task:** backporting this repo's own `vm-bootstrap`
  ...
  guidance) improvements down into the downstream project's installed copy — deliberately out of scope
